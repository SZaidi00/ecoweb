...
Every entry has exactly the fields `id`, `name`, `biome`, `location`, `nodeCount`, `provenance`, `tagline`, `lat`, `lng`. The validator cross-checks each entry against the actual web file: `id` must match the file name (`<id>.json`), `name`/`biome`/`location`/`provenance`/`tagline`/`lat`/`lng` must match the file's `meta`, and `nodeCount` must equal the file's actual node count. `lat`/`lng` are the study site's representative coordinates (degrees) and drive the map pin on the landing page. Every web file in `data/webs/` must be registered. The file is generated by `scripts/pipeline/build_index.py` (`npm run build:index`) — never hand-edit it.
